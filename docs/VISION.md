...
- [x] **v7 — API injection + crypto + info disclosure**: 188 rules — GraphQL (introspection, depth/complexity limits), WebSocket origin, prototype pollution, NoSQL/LDAP/template injection (SSTI), SSRF expanded (urlopen, axios, fetch, file_get_contents), crypto misuse (ECB, hardcoded IV/salt, short AES/RSA keys, PKCS1v1.5 padding, deprecated crypto.random, MD5/SHA1 for passwords in 5 languages), PHP injection patterns (SQL/eval/include/unserialize with $_GET/$_POST), info disclosure (debug code, hardcoded IPs, HTTP URLs, verbose errors, file upload).
- [x] **v8 — language deep dives + mobile + cloud**: 235 rules across 25 languages — C/C++ (format strings, scanf, alloca, strtok, atoi), Rust (transmute, from_raw, raw pointers, ptr::read/write), Go (unsafe.Pointer, cgo, Sprintf-SQL, no-TLS), Java (JNDI/Log4Shell, SpEL, XPath, SSRF, TransformerFactory), Android (debuggable, cleartext, backup, WebView, exported), iOS (ATS, plist secrets), Rails (CSRF skip, render SSTI, attr_accessible), Laravel (DB::raw, Blade {!!}, env()), AWS (S3 public-read, IAM wildcard, hardcoded creds), database (Redis EVAL, ES query_string, ORDER BY), network (FTP/Telnet/SMTP cleartext), supply chain (GHA @main, npm postinstall, pip git).
